...
class ActionMineCoopHands : ActionContinuousBase
{
	...
	override void CreateConditionComponents()
	{
		...
		//m_ConditionTarget = new CCTObject(2); // meters distance from object
	}
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		else
		{
			...
			{
				ZenCoops_SendMsg(action_data.m_Player, "ERROR: No valid chicken coop object found - in debug mode getting feathers won't work!");
			}
		...
		if (feathersLeft <= 0)
		{
			ZenCoops_SendMsg(action_data.m_Player, GetZenChickenCoopsConfig().NoFeathersLeftMessage);
			return;
		}
	...
	void ZenCoops_SendMsg(PlayerBase player, string message)
	{
		if (message == "")
			return;
